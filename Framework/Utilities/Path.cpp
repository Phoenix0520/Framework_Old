...
const WCHAR* Path::imageFilter			= L"이미지 파일(.png, .bmp, .jpg)\0*.png;*.bmp;*.jpg";
const WCHAR* Path::binModelFilter		= L"바이너리 모델 파일(.model)\0*.model";
const WCHAR* Path::fbxModelFilter		= L"Fbx 모델 파일(.fbx, .obk)\0*.fbx;*.obj\0";
const WCHAR* Path::shaderFilter			= L"셰이더 파일\0*.hlsl;*.fx";
...
wstring Path::OpenFileDialog(wstring file, wstring folder, const WCHAR* filter, function<void(wstring)> func, HWND hwnd)
{
	WCHAR name[255];
...
wstring Path::SaveFileDialog(wstring file, wstring folder, const WCHAR* filter, function<void(wstring)> func, HWND hwnd)
{
	WCHAR name[255];
	wcscpy_s(name, file.c_str());
